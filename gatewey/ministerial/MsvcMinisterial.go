...
func MsvcMinisterial(c *fiber.Ctx) error {
	EnvLoad()
	MSVC_MINISTERIAL_URL := os.Getenv("MSVC_MINISTERIAL_URL")
...
func MsvcMinisterialUser(c *fiber.Ctx) error {
	EnvLoad()
	MSVC_MINISTERIAL_USER_URL := os.Getenv("MSVC_MINISTERIAL_USER_URL")
	id := c.Params(utils.ID)
